...
class MemoryEfficientSeparator:
    ...
    async def process_stage(self, input_path, model_name, temp_dir, stage_name):
        ...
        if stage_name == "Denoising":
            input_path = input_path.replace("(No Reverb)", "(Instrumental)")
    ...
    def _get_correct_input_path(self, stage_name, input_path):
        """스테이지에 따른 올바른 입력 파일 경로 반환"""
        if stage_name == "Denoising":
            return input_path.replace("(No Reverb)", "(Instrumental)")
        return input_path
