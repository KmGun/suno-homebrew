...
async def websocket_endpoint(websocket: WebSocket):
    logger.debug("웹소켓 엔드포인트 진입")
    try:
        await websocket.accept()
        song_request_id = None
        
        logger.debug("웹소켓 연결 수락됨")
        logger.debug("클라이언트로부터 메시지 대기 중...")
        
        try:
            logger.debug("메시지 수신 시도...")
            raw_data = await websocket.receive()  # text나 bytes로 먼저 받아보기
            logger.debug(f"수신된 raw 데이터: {raw_data}")
            
            data = await websocket.receive_json()
            logger.debug(f"수신된 데이터 타입: {type(data)}")
            logger.debug(f"수신된 데이터 내용: {data}")
        except Exception as e:
            logger.error(f"메시지 수신 중 에러 발생: {str(e)}")
