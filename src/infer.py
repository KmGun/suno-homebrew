...
def infer_ai_cover(guide_path, voice_model, song_request_id, index):
    ...
    try:
        ...
        for pitch_value in model_pitch_values:
            # AI 보컬 생성
            output_filename = f"[{pitch_value}]aivocal{index}.mp3"
            ...
            try:
                ...
                if not os.path.exists(output_path):
                    logger.error(f"AI vocal file not generated: {output_path}")
                    continue

                # 리버브 적용
                reverb_filename = f"[{pitch_value}]reverb{index}.mp3"
                reverb_path = os.path.join(result_folder, reverb_filename)
                apply_reverb(output_path, reverb_path)

                # 리버브 파일이 생성되었는지 확인
                if os.path.exists(reverb_path):
                    reverb_pairs.append(reverb_path)
                else:
                    logger.error(f"Reverb file not generated: {reverb_path}")

            except Exception as e:
                logger.error(f"Error processing pitch {pitch_value}: {str(e)}")
                continue

        if not reverb_pairs:
            logger.error("No reverb files were generated successfully")
            return []

        logger.info(f"추론 완료: {voice_model}, 생성된 리버브 파일 수: {len(reverb_pairs)}")
        return reverb_pairs
