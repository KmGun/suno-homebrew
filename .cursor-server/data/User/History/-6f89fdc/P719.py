...
def download_google_drive_folder(service, folder_id, local_path, folder_name=None):
    """
    Google Drive의 특정 폴더를 로컬에 ��운로드합니다.
...
def infer_ai_cover(guide_path, voice_model, isMan):
    ...
    try:
        ...
        song_urls = []
        result_folder = f"./temp/{os.path.basename(guide_path).split('.')[0]}"
        ...
        for pitch_value in model_pitch_values:
            ...
            output_path = f"{result_folder}/vocal_{pitch_value}.mp3"
            try:
                voice_change(
                    voice_model,
                    guide_path,
            ...
            ai_vocal_path = apply_reverb(result_folder, pitch_value, "vocal.mp3")
