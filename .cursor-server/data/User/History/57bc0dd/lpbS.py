...
def main():
    """메인 실행 함수"""
    try:
        processor = AICoverProcessor()
        
        # 실행 예시
        guide_path = "/path/to/local/guide.wav"  # 로컬 가이드 파일 경로
        model_name = "your_model_name"           # 사용할 모델명
        is_male = True                           # 남성 보컬 여부

        if not processor.process_cover(guide_path, model_name, is_male):
            logger.error("Failed to process cover")
            return False
