...
from src.post_process_audio import apply_reverb, mix_audio
...
def infer_ai_cover(guide_path, voice_model, song_request_id):
    ...
    try:
        ...
        song_urls = []
        result_folder = os.path.join("/app/temp", song_request_id, os.path.splitext(os.path.basename(guide_path))[0])
        os.makedirs(result_folder, exist_ok=True)

        # 추론 시작
        for pitch_value in model_pitch_values:
            check_audio_samplerate(guide_path, "Before voice_change - Input vocal")
            
            output_path = os.path.join(result_folder, f"vocal_{pitch_value}.mp3")
            try:
                voice_change(
                    voice_model,
                    guide_path,
            ...
            ai_vocal_path = apply_reverb(result_folder, f"vocal_{pitch_value}.mp3")
            check_audio_samplerate(ai_vocal_path, "After reverb")

            audioPair = {
                "local_path": ai_vocal_path,
                "file_name": f"vocal_{pitch_value}_reverb.mp3"
            }
            song_urls.append(audioPair)

        logger.info(f"추론 완료: {voice_model}")
        return result_folder, song_urls
