...
class MemoryEfficientSeparator:
    ...
    async def separate_audio_tracks(self, input_file_path, song_request_id, file_number):
        ...
        current_file_name = os.path.basename(temp_file_path).replace('.mp3', '')
        ...
        for model_name, stage_name in stages:
            ...
            if stage_name == "Kim Vocal 1 separation":
                current_input = f"{temp_dir}/{current_file_name}_(Vocals)_Kim_Vocal_1.mp3"
            elif stage_name == "6HP-Karaoke separation":
                current_input = f"{temp_dir}/{current_file_name}_(Vocals)_Kim_Vocal_1_(Vocals)_6_HP-Karaoke-UVR.mp3"
            elif stage_name == "Reverb application":
                current_input = f"{temp_dir}/{current_file_name}_(Vocals)_Kim_Vocal_1_(Vocals)_6_HP-Karaoke-UVR_(No Reverb)_Reverb_HQ_By_FoxJoy.mp3"
        ...
        # 실제 생성된 파일 확인 후 복사
        vocal_path = f"{temp_dir}/{current_file_name}_(Vocals)_Kim_Vocal_1_(Vocals)_6_HP-Karaoke-UVR_(No Reverb)_Reverb_HQ_By_FoxJoy_(Instrumental)_UVR-DeNoise.mp3"
        mr_path = f"{temp_dir}/{current_file_name}_(Instrumental)_Kim_Vocal_1.mp3"
        chorus_path = f"{temp_dir}/{current_file_name}_(Vocals)_Kim_Vocal_1_(Instrumental)_6_HP-Karaoke-UVR.mp3"
        
        # 파일 존재 여부 확인 후 복사
        if os.path.exists(mr_path):
            shutil.copy2(mr_path, result_paths['mr'])
        if os.path.exists(chorus_path):
            shutil.copy2(chorus_path, result_paths['chorus'])
        if os.path.exists(vocal_path):
            shutil.copy2(vocal_path, result_paths['vocal'])
        else:
            raise FileNotFoundError(f"처리된 파일을 찾을 수 없습니다: {vocal_path}")
