...
#     hubert_model = load_hubert(device, config.is_half, "/app/rvc_models/hubert_base.pt")
...
def voice_change(
    ...
    try:
        logger.debug(f"Starting voice_change with parameters:")
        logger.debug(f"voice_model: {voice_model}")
        logger.debug(f"vocals_path: {vocals_path}")
        logger.debug(f"output_path: {output_path}")
        logger.debug(f"pitch_change: {pitch_change}")

        # 파일 존재 확인 및 ��보 로깅
        logger.debug(f"Checking input file: {vocals_path}")
        if os.path.exists(vocals_path):
            logger.debug(f"Input file exists. Size: {os.path.getsize(vocals_path)}")
            logger.debug(f"File permissions: {oct(os.stat(vocals_path).st_mode)[-3:]}")
        else:
            logger.error(f"Input file does not exist: {vocals_path}")
            raise FileNotFoundError(f"Input file not found: {vocals_path}")

        # RVC 모델 로드
        logger.debug(f"Loading RVC model for {voice_model}")
        rvc_model_path, rvc_index_path = get_rvc_model(voice_model, is_webui)
        logger.debug(f"RVC model path: {rvc_model_path}")
        logger.debug(f"RVC index path: {rvc_index_path}")
        ...
        # Hubert 모델 로드
        logger.debug("Loading Hubert model")
        try:
            hubert_model = load_hubert(
                device, config.is_half, "/app/rvc_models/hubert_base.pt"
            )
            logger.debug("Hubert model loaded successfully")
        except Exception as e:
            logger.error(f"Failed to load Hubert model: {str(e)}")
            raise

        # VC 모델 로드
        logger.debug("Loading VC model")
        try:
            cpt, version, net_g, tgt_sr, vc = get_vc(
                device, config.is_half, config, rvc_model_path
            )
            logger.debug(f"VC model loaded successfully. Version: {version}")
        except Exception as e:
            logger.error(f"Failed to load VC model: {str(e)}")
            raise

        # RVC 추론 실행
        logger.debug("Starting RVC inference")
        try:
            rvc_infer(
                rvc_index_path,
                index_rate,
            ...
            logger.debug("RVC inference completed successfully")
        except Exception as e:
            logger.error(f"RVC inference failed: {str(e)}")
            if hasattr(e, "stderr"):
        ...
        del hubert_model, cpt
        gc.collect()
        logger.debug("Voice change completed successfully")

    except Exception as e:
        logger.error(f"Error in voice_change: {str(e)}")
        if hasattr(e, "stderr"):
            logger.error(f"Stderr output: {e.stderr}")
        raise
...
def separate_audio_tracks(input_file_path, song_request_id, file_number):
    """
    ...
    subprocess.run([
        sys.executable, "-m", "audio_separator", temp_file_path, 
        "-m", "Kim_Vocal_1.onnx",
        "--output_dir", temp_dir,
        "--output_format", "mp3",
        "--normalization", "0.9",
    ...
    subprocess.run([
        sys.executable, "-m", "audio_separator", vocals_path,
    ...
    subprocess.run([
        sys.executable, "-m", "audio_separator", karaoke_vocals_path,
    ...
    subprocess.run([
        sys.executable, "-m", "audio_separator", reverb_path,
        "-m", "UVR-DeNoise.pth",
        "--output_dir", temp_dir,
        "--output_format", "mp3",
        "--normalization", "0.9",
...
def song_cover_pipeline(
    ...
    try:
        if not zip_path or not voice_model:
            raise_exception("모델이나 압축파일 둘중 하나를 선택 완료하세요", is_webui)

        display_progress(
            "[~] Starting AI Cover Generation Pipeline...", 0, is_webui, progress
        )

        with open(os.path.join(mdxnet_models_dir, "model_data.json")) as infile:
            mdx_model_params = json.load(infile)
        ######################### 새로추가된 코드 ##########################
        root_output_dir = os.path.join(output_dir, "no_others")
        if not os.path.exists(root_output_dir):
            os.makedirs(root_output_dir)

        infer_output_dir = os.path.join(output_dir, "infer")
        if not os.path.exists(infer_output_dir):
            os.makedirs(infer_output_dir)

        # 보컬 파일만 찾아오기.
        input_song_titles = ["곡1", "곡2"]
        now_song_title = "곡1"

        vocal_file_paths = find_vocal_files(input_song_titles)
        # print(vocal_file_paths)

        file_paths = vocal_file_paths

        # 어떤 가수 모델 쓸건지?
        model_list = ["iu_new", "kimdr", "imchangjung", "kgs"]
        new_voice_model
        ...
        for file_path in file_paths:
            ...
            ####### 추론 결과물 경로 생성
            ai_vocals_path = (
                f"/content/drive/MyDrive/infer/{now_song_title}/{file_name}.wav"
            )

            ####### 추론
            if not os.path.exists(ai_vocals_path):
                # display_progress(
                #     "[~] Converting voice using RVC...", 0.5, is_webui, progress
                # )
                voice_change(
                    new_voice_model,
                    file_path,
