...
class MemoryEfficientSeparator:
    ...
    def _create_separator(self, temp_dir):
        ...
        # GPU 메모리 상태에 따라 배치 사이즈와 오버랩 조정
        if torch.cuda.is_available():
            free_memory = torch.cuda.get_device_properties(0).total_memory - torch.cuda.memory_allocated(0)
            
            # 메모리 상태에 따른 더 세분화된 설정
            if free_memory < 1 * (1024 ** 3):  # 1GB 미만
                params['batch_size'] = 0.25     # 배치 사이즈를 0.25로 대폭 감소
                params['overlap'] = 0.01        # 최소한의 오버랩
            elif free_memory < 2 * (1024 ** 3): # 2GB 미만
                params['batch_size'] = 0.5      # 배치 사이즈를 0.5로 감소
                params['overlap'] = 0.03
            elif free_memory < 4 * (1024 ** 3): # 4GB 미만
                params['batch_size'] = 0.75     # 배치 사이즈를 0.75로 설정
                params['overlap'] = 0.05
            else:
                params['batch_size'] = 1        # 충분한 메모리가 있을 때
                params['overlap'] = 0.1
